...
public class DbStorageService implements StorageService {
    ...
    private class DbMinerStore implements MinerStore {
        ...
        public List<Deadline> getDeadlines() { // TODO cache
            return useDslContext(context -> context.select(MINER_DEADLINES.BASE_TARGET, MINER_DEADLINES.SHARE_PERCENT, MINER_DEADLINES.HEIGHT, MINER_DEADLINES.DEADLINE, MINER_DEADLINES.COMMITMENT_FACTOR, MINER_DEADLINES.DEADLINE_WITHOUT_FACTOR, MINER_DEADLINES.COMMITMENT)
                    .from(MINER_DEADLINES)
                    .where(MINER_DEADLINES.ACCOUNT_ID.eq(accountId))
                    .fetch()
                    .map(record -> new Deadline(BigInteger.valueOf(record.get(MINER_DEADLINES.DEADLINE)), BigInteger.valueOf(record.get(MINER_DEADLINES.DEADLINE_WITHOUT_FACTOR)), BigInteger.valueOf(record.get(MINER_DEADLINES.BASE_TARGET)), record.get(MINER_DEADLINES.SHARE_PERCENT), record.get(MINER_DEADLINES.HEIGHT), record.get(MINER_DEADLINES.COMMITMENT_FACTOR), record.get(MINER_DEADLINES.COMMITMENT))));
        }
        ...
        public Deadline getDeadline(long height) {
            try {
                return getFromCacheOr(MINER_DEADLINES, accountIdStr + "deadline" + Long.toString(height), () -> useDslContext(context -> context.select(MINER_DEADLINES.BASE_TARGET, MINER_DEADLINES.SHARE_PERCENT, MINER_DEADLINES.HEIGHT, MINER_DEADLINES.DEADLINE, MINER_DEADLINES.COMMITMENT_FACTOR, MINER_DEADLINES.DEADLINE_WITHOUT_FACTOR, MINER_DEADLINES.COMMITMENT)
                        .from(MINER_DEADLINES)
                        .where(MINER_DEADLINES.ACCOUNT_ID.eq(accountId), MINER_DEADLINES.HEIGHT.eq(height))
                        .fetchAny()
                        .map(record -> new Deadline(BigInteger.valueOf(record.get(MINER_DEADLINES.DEADLINE)), BigInteger.valueOf(record.get(MINER_DEADLINES.DEADLINE_WITHOUT_FACTOR)), BigInteger.valueOf(record.get(MINER_DEADLINES.BASE_TARGET)), record.get(MINER_DEADLINES.SHARE_PERCENT), height, record.get(MINER_DEADLINES.COMMITMENT_FACTOR), record.get(MINER_DEADLINES.COMMITMENT)))));
            } catch (NullPointerException e) {
                return null;
            }
        }
        ...
        public void setOrUpdateDeadline(long height, Deadline deadline) {
            useDslContextVoid(context -> context.mergeInto(MINER_DEADLINES, MINER_DEADLINES.ACCOUNT_ID, MINER_DEADLINES.SHARE_PERCENT, MINER_DEADLINES.HEIGHT, MINER_DEADLINES.DEADLINE, MINER_DEADLINES.BASE_TARGET, MINER_DEADLINES.COMMITMENT_FACTOR, MINER_DEADLINES.DEADLINE_WITHOUT_FACTOR, MINER_DEADLINES.COMMITMENT)
                    .key(MINER_DEADLINES.ACCOUNT_ID, MINER_DEADLINES.HEIGHT)
                    .values(accountId, deadline.getSharePercent(), height, deadline.getDeadline().longValue(), deadline.getBaseTarget().longValue(), deadline.getCommitmentFactor(), deadline.getDeadlineWithoutFactor().longValue(), deadline.getCommitment())
                    .execute());
